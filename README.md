...
## 5. Схема БД
![Пример изображения](bd2.png)

Оформление заказа. Запись в таблицу ride
| Таблица             | Размер одной строки | Кол-во записей | Объем |
|---------------------|---------------------|----------------|-------|
| ride                | |  |  |
| driver              | |  |  |
| passanger           | |  |  |
...
`Итого 32 б * 40к = 1250 Кб`

Просмотр информации о заказе. Чтение из таблиц ride, driver, garage, car, type
| Поле в бд        | Размер|
|------------------|-------|
| id               | 4 б   |
| driver_name      | 30 б  |
...
`Итого 448 б * 80 = 35 Кб`

Оценить заказ. Запись в аблицу rating
| Поле в бд | Размер|
|-----------|-------|
| id        | 4 б |
| rating    | 2 б   |
...
Запись обновленного рейтинга водителя в таблицу driver 2 б<br>
`Итого 112 б * 40 = 4,4 Кб`
